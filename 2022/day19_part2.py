...
def cullBadChoices(paths, n):
    ...
    for (robots, supplies, target) in paths:
        geodes = robots[3]*(32-n) + supplies[3]
        geodeRate = int(np.ceil(robots[3]/n))
        if geodeRate:
            numExtra = 1
            for ng in range(n+geodeRate, 32, geodeRate):
                geodes += numExtra
                numExtra += 1
        if geodes > mostGeodes:
            mostGeodes = geodes
        numGeodeRobots = max(numGeodeRobots, robots[3])
    newpaths = []
    for path in paths:
        (robots, supplies, target) = path
        possiblegeodes = supplies[3] + robots[3]*(32-n) + sum(range(n))
        if possiblegeodes >= mostGeodes:
            if numGeodeRobots-1 > robots[3]:
                #Someone beat us to making a geode robot while we were waiting for one.
                continue
...
for b in range(min(3, len(blueprints))):
    ...
    for n in range(32):
        ...
        #paths = newpaths
        # printPaths(paths)
        print("Number of options:", len(paths))
    # printPaths(paths)
    mostObsidian = 0
    for option in newpaths:
        if option[1][3] > mostObsidian:
            mostObsidian = option[1][3]
    qualityScores.append(mostObsidian)
    print("Most obsidian:", mostObsidian)

print(qualityScores)
print(qualityScores[0] * qualityScores[1] * qualityScores[2])
